...
class TestCTS(unittest.TestCase):
    ...
    def test_connect_to_service(self):
        ...
        response = asyncio.run(run_in_session(self.converter, 'query_the_service', ['CTS', args]))
        ...
        try:
            asyncio.run(run_in_session(self.converter, 'query_the_service', ['random', '']))
    ...
    def test_cas_to_inchikey(self):
        ...
        cas_number = '7783-89-3'
        self.assertEqual(asyncio.run(run_in_session(self.converter, 'cas_to_inchikey', [cas_number])), inchikey)

        cas_number = '7783893'
        self.assertIsNone(asyncio.run(run_in_session(self.converter, 'cas_to_inchikey', [cas_number])))
    ...
    def test_inchikey_to_IUPAC_name(self):
        ...
        asyncio.run(run_in_session(self.converter, 'inchikey_to_iupac_name', [inchikey]))
        self.assertEqual(asyncio.run(run_in_session(self.converter, 'inchikey_to_iupac_name', [inchikey])), uipac_name)

        inchikey = 'XQLMNMQIKR-UHFFFAOYSA-M'
        self.assertIsNone(asyncio.run(run_in_session(self.converter, 'inchikey_to_iupac_name', [inchikey])))
