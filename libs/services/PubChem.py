...
class PubChem(Converter):
    ...
    async def name_to_inchi(self, name, session):
        """
        ...
        response = await self.query_the_service('PubChem', args, session)
        if response:
    ...
    async def inchi_to_inchikey(self, inchi, session):
        """
        ...
        """
        props = await self.get_props_from_inchi(inchi, session)
        if props:
            for prop in props:
                if prop['urn']['label'] == 'InChIKey':
                    return prop['value']['sval']

    async def inchi_to_iupac_name(self, inchi, session):
        """
    ...
    async def inchi_to_formula(self, inchi, session):
        """
    ...
    async def inchi_to_smiles(self, inchi, session):
        """
    ...
    async def get_props_from_inchi(self, inchi, session):
        """
        ...
        """
        args = "inchi/JSON"
        response = await self.query_the_service('PubChem', args, session, method='POST', data={'inchi': inchi})
        if response:
            response_json = json.loads(response)
